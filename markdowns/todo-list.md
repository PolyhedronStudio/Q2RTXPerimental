...
#### Entity Collision Hulls:
    - [ ] (Priority: Low) Store an identity, and transformed hull for each `server_entity_t` which are updated when linking the entity.
...
#### Technically:
- [ ] Finish the CCD entity shape tracing to work properly.
    - [X] BUGFIX: Capsule interaction when height Z axis touching differs seems a little sketchy.
    - [ ] Test a SPHERE entity.
    - [ ] Test a cylinder only entity.
- [X] Implement Recast/Detour libraries for AI.
    - [/] We are implementing our own navmesh system using a KD-Tree and Half-Edge Mesh N-Gon representations.
        - [X] Basic generation and traversal with a basic API.
        - [ ] Means of Squad/Crowd control(Implement as second system on top of `nav` system)
        - [X] Support for func_door/func_door_rotating.
        - [ ] Support for func_rotating.
        - [ ] Support for func_plat.
        - [ ] Support for func_train.
...
### Asset/Pipeline Modification Work:
	- [ ] Modify ericw-tools to accept our own FBSP inspired format with patches based on Q3 .map(v220)
	- [ ] Allow for setting hitbox capsules by hand in iqmtools.
...
---
```
# THIS IS THE WAY WAY OLD TODO LIST WHICH STILL CONTAINTS PERHAPS USEFUL REMNANTS, FULLY IGNORE OTHERWISE!!!!11111 [[ 1337 g4m3d3vs only 4 life!!11!1 Shout out to y'all!111!1 ]]
```
---
---
Just some space here, to separate lol, cba to search markdown tag for that atm.
...
---
## General TODO:
* [ ] STREAMLINE: The code for server, and server game so that the imports/exports are properly cased,
...
* [ ] IMPLEMENT: Wren instead of Lua for game logic scripting.
    - [X] Look into Wren embedding.
    - [X] Look into Wren C API.
        - [X] WrenBind for C++ 17. https://matusnovak.github.io/wrenbind17/
			Epic library!
    - [X] Look into Wren memory management.
    - [X] Look into Wren module system.
    - [X] Look into Wren performance.
		Epic language!
    - [X] Look into Wren debugging support.
    - [X] Look into Wren community support.
    - [X] Look into Wren documentation quality.
...
* [ ] --
* [X] FIX: Pushers(rotating to be specific), we get stuck, need separation of origins.
    - [X] Look into the actual origin Q3 has that is used for collision and those of rendering.
        ...
        - [X] Adjusted all places where an entity's origin/angles are set to use these functions instead.
            ...
            - [X] Look into all entities that move, and ensure they use these functions.
                - [X] Ensure that currentOrigin is set at spawn, as well as for the player move
                mechanics.
                - [X] Ensure that whenever SVG_Trace is used, it uses currentOrigin/currentAngles.
                - [X] Patch the following entities to use currentOrigin/currentAngles for collision tracing:
                    - [X] func_door
                    ...
                    - [X] func_train
                    - [X] func_wall
                 - [X] Patch the player move mechanics to use currentOrigin/currentAngles for collision tracing.
...
* [X] Look into a simple UI layout system which we can use for HUD and other UI elements.
    * [X] - Picked "MicroUI" for now as it seems easy to use, minimal, and extensible. It is also MIT licensed.
...
* [X] Implement client game cvars for debugging things more appropriately.
	...
	* [X] Bounding Box Display.
	    - [X] Bounding Box Display for all entities.
        - [X] Spheres, Capsules, and Cylinders.
        - [X] Arrow for direction of movement.
* [ ] --
---
...
animations for open/closed/transit-in/transit-out states throughout TB editor. (``func_button,func_door etc``)
    - [ ] Use a Q3 "Shader" style approach for this, and allow for a simple shader script to be used for this.
    In doing so we can apply special effects such as a warp or masking effect to the texture as well. (Think of a forcefield or a shield, etc)
---
...
* [X] Find the bug that is currently making the OctagonHull not enjoy colliding to certain specific bounding boxes.
...
- [ ] **Entities**:
	- [X] Rewrite/Rework/Fix func_plat. It needs a different set of usage rules._
...
- [ ] **VKPT:**
	- [X] Port over/move in the debug drawing code of Q2RTX recent commits.
			We got our own now.
...
- [ ] **MethLib Scenario:** (Pun intended)
	- [ ] Still got to redo the whole thing inspired by glMatrix.net
...
- [ ] **The Monster Scenario:**
	- [ ] Allow a callback for monsters during waypoint arrival to filter out waypoints and/or switch NPC action in general.

- [ ] **EDITOR/VIEWER Scenario:**
