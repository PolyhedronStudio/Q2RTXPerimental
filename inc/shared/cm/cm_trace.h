...
**/
	...
	typedef struct trace_s {
		...
		**/
		...
		//! First Surface hit.
		const cm_surface_t *surface = nullptr;
		//! First Surface collision model material pointer.
		const struct cm_material_s *material = nullptr;
		...
		**/
		...
		//! Second surface hit.
		const cm_surface_t *surface2 = nullptr;
		//! Second surface collision model material pointer.
		const struct cm_material_s *material2 = nullptr;
	} cm_trace_t;
#else
	typedef struct trace_s {
		//! If true, the entire trace took place within a solid(brush) area.
