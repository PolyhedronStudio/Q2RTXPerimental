...
void boss2_attack(edict_t *self)
{
    ...
    if (range <= 125) {
        M_SetAnimation( self, &boss2_move_attack_pre_mg );
    } else {
        if (random() <= 0.6f)
            M_SetAnimation( self, &boss2_move_attack_pre_mg );
        else
            M_SetAnimation( self, &boss2_move_attack_rocket );
    }
}

void boss2_attack_mg(edict_t *self)
{
    M_SetAnimation( self, &boss2_move_attack_mg );
}
...
void boss2_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
    ...
    self->count = 0;
<<<<<<<< HEAD:src/baseq2/svgame/m_boss2.cpp
...
>>>>>>>> 32d0fe4cb25722ded82c772b022dcafe9ad01cb6:src/game/m_boss2.c
}
