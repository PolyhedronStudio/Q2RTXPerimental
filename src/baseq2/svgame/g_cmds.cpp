...
static bool FloodProtect(edict_t *ent)
{
	...
	if ( flood_msgs->value ) {
		...
		if ( cl->flood_when[ i ] &&
			level.time - cl->flood_when[ i ] < QMTime::FromMilliseconds( flood_persecond->value ) ) {
			cl->flood_locktill = level.time + QMTime::FromMilliseconds( flood_waitdelay->value );
			gi.cprintf(
...
void ClientCommand(edict_t *ent)
{
    ...
    if (Q_stricmp(cmd, "use") == 0)
        ...
        SVG_Inventory_SelectNextItem(ent, IT_WEAPON);
    else if (Q_stricmp(cmd, "invprevw") == 0)
        SVG_Inventory_SelectPrevItem(ent, IT_WEAPON);
    else if (Q_stricmp(cmd, "invnextp") == 0)
        SVG_Inventory_SelectNextItem(ent, IT_POWERUP);
    else if (Q_stricmp(cmd, "invprevp") == 0)
        SVG_Inventory_SelectPrevItem(ent, IT_POWERUP);
