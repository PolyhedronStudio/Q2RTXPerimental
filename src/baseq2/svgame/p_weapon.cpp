...
static void SVG_Player_ProjectSource(edict_t* ent, vec3_t point, vec3_t distance, vec3_t forward, vec3_t right, vec3_t result)
{
    vec3_t  _distance;
...
inline QMTime Weapon_AnimationTime( edict_t *ent ) {
	...
	return QMTime::FromMilliseconds( ( 1.f / ent->client->ps.gunrate ) * 1000 );
}
...
Called by SVG_Client_BeginServerFrame and ClientThink
=================
*/
...
inline bool Weapon_CanAnimate( edict_t *ent ) {
	...
	return !ent->lifeStatus && ent->s.modelindex == MODELINDEX_PLAYER;
}
...
constexpr QMTime GRENADE_TIMER = 3_sec;
...
void weapon_grenade_fire(edict_t *ent, bool held)
{
    ...
    SVG_Player_ProjectSource(ent, ent->s.origin, offset, forward, right, start);

	QMTime timer = ent->client->grenade_time - level.time;
    ...
    if (ent->lifeStatus || ent->s.modelindex != 255) { // VWep animations screw up corpses
        return;
    }
...
void weapon_grenadelauncher_fire(edict_t *ent)
{
    ...
    gi.WriteUint8(MZ_GRENADE | is_silenced);
...
void Weapon_RocketLauncher_Fire(edict_t *ent)
{
    ...
    gi.WriteUint8(svc_muzzleflash);
    gi.WriteInt16(ent - g_edicts);
    gi.WriteUint8(MZ_ROCKET | is_silenced);
    gi.multicast( ent->s.origin, MULTICAST_PVS, false  );
...
void Blaster_Fire(edict_t *ent, const vec3_t g_offset, int damage, bool hyper, int effect)
{
    ...
    gi.WriteInt16(ent - g_edicts);
    if (hyper)
        gi.WriteUint8(MZ_HYPERBLASTER | is_silenced);
    else
        gi.WriteUint8(MZ_BLASTER | is_silenced);
    gi.multicast( ent->s.origin, MULTICAST_PVS, false );
...
void Chaingun_Fire(edict_t *ent)
{
    ...
    gi.WriteUint8(svc_muzzleflash);
    gi.WriteInt16(ent - g_edicts);
    gi.WriteUint8((MZ_CHAINGUN1 + shots - 1) | is_silenced);
    gi.multicast( ent->s.origin, MULTICAST_PVS, false );
...
void weapon_supershotgun_fire(edict_t *ent)
{
    ...
    gi.WriteUint8(svc_muzzleflash);
    gi.WriteInt16(ent - g_edicts);
    gi.WriteUint8(MZ_SSHOTGUN | is_silenced);
    gi.multicast( ent->s.origin, MULTICAST_PVS, false );
...
void weapon_railgun_fire(edict_t *ent)
{
    ...
    gi.WriteUint8(svc_muzzleflash);
    gi.WriteInt16(ent - g_edicts);
    gi.WriteUint8(MZ_RAILGUN | is_silenced);
    gi.multicast( ent->s.origin, MULTICAST_PVS, false );
...
void weapon_bfg_fire(edict_t *ent)
{
    ...
    if (ent->client->ps.gunframe == 9) {
        ...
        gi.WriteUint8(svc_muzzleflash);
        gi.WriteInt16(ent - g_edicts);
        gi.WriteUint8(MZ_BFG | is_silenced);
        gi.multicast( ent->s.origin, MULTICAST_PVS, false );
...
void weapon_flaregun_fire(edict_t *ent)
{
	...
	gi.WriteUint8(svc_muzzleflash);
	gi.WriteInt16(ent - g_edicts);
	gi.WriteUint8(MZ_FLARE | is_silenced);
	gi.multicast( ent->s.origin, MULTICAST_PVS, false );
