...
const int32_t PM_StepSlideMove_Generic( Vector3 &origin, Vector3 &velocity, const float frametime, const Vector3 &mins, const Vector3 &maxs, pm_touch_trace_list_t &touch_traces, const bool has_time ) {
	...
	for ( bumpcount = 0; bumpcount < numbumps; bumpcount++ ) {
		...
		if ( trace.surface2 ) {
			...
			PM_BounceClipVelocity( velocity, trace.plane.normal, clipped_a, 1.01f );
			PM_BounceClipVelocity( velocity, trace.plane2.normal, clipped_b, 1.01f );
		...
		int32_t j = 0;
		for ( i = 0; i < numplanes; i++ ) {
			PM_BounceClipVelocity( velocity, planes[ i ], velocity, 1.01f );
