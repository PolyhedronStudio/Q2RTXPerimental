...
*
*
*	ServerGame: sg_string_t type, dynamically allocates the required
...
struct sg_qstring_t {
	...
	**/
	constexpr sg_qstring_t( const T *charStr ) {
		if ( !charStr ) {
			ptr = nullptr;
			count = 0;
			return;
...
private:
	...
	**/
	constexpr sg_qstring_t( T *ptr, size_t count ) :
		ptr( ptr ),
		count( count ) {
	}
...
public:
	...
	**/
	constexpr sg_qstring_t( const sg_qstring_t &charstr ) {
		*this = sg_qstring_t<T, tag>::from_char_str( (const T *)charstr.ptr );
	}
	/**
	*	@brief	Will allocate enough space, and copy in the contents of charstr.
	**/
	constexpr sg_qstring_t &operator=( const sg_qstring_t &charstr ) {
	...
	**/
	constexpr sg_qstring_t( sg_qstring_t &&move ) {
		ptr = move.ptr;
		count = move.count;
	...
	**/
	constexpr sg_qstring_t &operator=( sg_qstring_t &&move ) {
		ptr = move.ptr;
		count = move.count;
	...
	**/
	constexpr sg_qstring_t &operator=( const T *charStr ) {
		// Return this.
		if ( !charStr ) {
			// Release.
			release();
	...
	**/
	//inline sg_qstring_t<T, tag> from_char_str( const char *charStr ) {
	static inline sg_qstring_t<T, tag> from_std_string( const std::string &charStr ) {
