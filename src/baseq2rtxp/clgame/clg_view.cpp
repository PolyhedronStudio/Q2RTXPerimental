...
static void CLG_AddViewWeapon( void ) {
    ...
    if ( gun_frame ) {
        ...
        if ( ops->gunindex != ps->gunindex ) { // just changed weapons, don't lerp from old
            clgi.client->weapon.frame = clgi.client->weapon.last_frame = ps->gunframe;
            clgi.client->weapon.server_time = clgi.client->servertime;
        } else if ( clgi.client->weapon.frame == -1 || clgi.client->weapon.frame != ps->gunframe ) {
            clgi.client->weapon.frame = ps->gunframe;
            clgi.client->weapon.last_frame = ops->gunframe;
            clgi.client->weapon.server_time = clgi.client->servertime;
        }

        const int32_t playerstate_gun_rate = ps->gunrate;
        const float gun_ms = 1.f / ( !playerstate_gun_rate ? 10 : playerstate_gun_rate ) * 1000.f;
        gun.backlerp = 1.f - ( ( clgi.client->time - ( (float)clgi.client->weapon.server_time - clgi.client->sv_frametime ) ) / gun_ms );
        clamp( gun.backlerp, 0.0f, 1.f );
        gun.frame = clgi.client->weapon.frame;
        gun.oldframe = clgi.client->weapon.last_frame;
        // WID: 40hz
