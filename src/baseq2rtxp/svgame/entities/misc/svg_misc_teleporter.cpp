...
*   @brief
**/
void teleporter_touch( svg_entity_t *self, svg_entity_t *other, const cm_plane_t *plane, cm_surface_t *surf ) {
    svg_entity_t *dest;
    int         i;

    if ( !other->client )
        return;
    dest = SVG_Find( NULL, FOFS_GENTITY( targetname ), (const char *)self->targetNames.target );
    if ( !dest ) {
