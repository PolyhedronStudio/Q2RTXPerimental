...
DEFINE_MEMBER_CALLBACK_TOUCH( svg_item_edict_t, onTouch_DropTempTouch )( svg_item_edict_t *ent, svg_base_edict_t *other, const cm_plane_t *plane, cm_surface_t *surf ) -> void {
	...
	// Only dispatch callback if both entities are active and not the same entity.
	bool dispatchCallback = SVG_Entity_IsActive( other ) && SVG_Entity_IsActive( ent ) && other != ent->owner;
