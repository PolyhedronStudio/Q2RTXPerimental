...
void plat_go_down( svg_entity_t *ent );

void plat_think_idle( svg_entity_t *ent ) {
    ent->think = plat_think_idle;
    ent->nextthink = level.time + FRAME_TIME_MS;
}

void plat_hit_top( svg_entity_t *ent ) {
    ...
    // Create temporary objects encapsulating access to svg_entity_t's.
...
void plat_hit_bottom( svg_entity_t *ent ) {
    if ( !( ent->flags & FL_TEAMSLAVE ) ) {
        if ( ent->pushMoveInfo.sounds.end )
            gi.sound( ent, CHAN_NO_PHS_ADD + CHAN_VOICE, ent->pushMoveInfo.sounds.end, 1, ATTN_STATIC, 0 );
        ent->s.sound = 0;
...
void plat_go_up( svg_entity_t *ent ) {
    if ( !( ent->flags & FL_TEAMSLAVE ) ) {
        if ( ent->pushMoveInfo.sounds.start )
            gi.sound( ent, CHAN_NO_PHS_ADD + CHAN_VOICE, ent->pushMoveInfo.sounds.start, 1, ATTN_STATIC, 0 );
        ent->s.sound = ent->pushMoveInfo.sounds.middle;
...
void plat_blocked( svg_entity_t *self, svg_entity_t *other ) {
...
void Use_Plat( svg_entity_t *ent, svg_entity_t *other, svg_entity_t *activator, const entity_usetarget_type_t useType, const int32_t useValue ) {
...
void Touch_Plat_Center( svg_entity_t *ent, svg_entity_t *other, cplane_t *plane, csurface_t *surf ) {
    if ( !other->client )
        return;
...
void plat_spawn_inside_trigger( svg_entity_t *ent ) {
    svg_entity_t *trigger;
    vec3_t  tmin, tmax;
