...
#define m_assert(condition, message) \
    do { \
        if (! (condition)) { \
            gi.bprintf( PRINT_ERROR, "Assertion '"#condition"' failed in "STRINGIFY(__FILE__)":"STRINGIFY2(__LINE__)": "message"\n"); \
        } \
    } while (false)
...
#define c_assert(condition) \
    do { \
        if (! (condition)) { \
            gi.bprintf( PRINT_ERROR, "Assertion '"#condition"' failed in "STRINGIFY(__FILE__)":"STRINGIFY2(__LINE__)"\n"); \
        } \
    } while (false)    
#endif  // #if0
#else
