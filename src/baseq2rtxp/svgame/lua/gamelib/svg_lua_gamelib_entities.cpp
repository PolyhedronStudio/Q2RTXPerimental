...
sol::userdata GameLib_GetEntityForLuaName( sol::this_state s, const std::string &luaName ) {
	...
	svg_edict_t *luaNameEntity = SVG_Find( NULL, FOFS_GENTITY( luaProperties.luaName ), luaName.c_str() );
	// Return it.
...
sol::userdata GameLib_GetEntityForTargetName( sol::this_state s, const std::string &targetName ) {
	...
	svg_edict_t *targetNameEntity = SVG_Find( NULL, FOFS_GENTITY( targetname ), targetName.c_str() );
	// Return it.
