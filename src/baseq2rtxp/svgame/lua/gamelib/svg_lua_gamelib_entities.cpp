...
sol::userdata GameLib_GetEntityForLuaName( sol::this_state s, const std::string &luaName ) {
	...
	svg_base_edict_t *luaNameEntity = SVG_Entities_Find( NULL, q_offsetof( svg_base_edict_t, luaProperties.luaName.ptr ), luaName.c_str() );
	// Return it.
...
sol::table GameLib_GetEntitiesForLuaName( sol::this_state s, const std::string &luaName ) {
	...
	while ( 1 ) {
		...
		luaNameEntity = SVG_Entities_Find( luaNameEntity, q_offsetof( svg_base_edict_t, luaProperties.luaName.ptr ), luaName.c_str() );
...
sol::userdata GameLib_GetEntityForTargetName( sol::this_state s, const std::string &targetName ) {
	...
	svg_base_edict_t *targetNameEntity = SVG_Entities_Find( NULL, q_offsetof( svg_base_edict_t, targetname.ptr ), targetName.c_str() );
	// Return it.
...
sol::table GameLib_GetEntitiesForTargetName( sol::this_state s, const std::string &targetName ) {
	...
	while ( 1 ) {
		// Find next entity in line.
		targetNameEntity = SVG_Entities_Find( targetNameEntity, q_offsetof( svg_base_edict_t, targetname.ptr ), targetName.c_str() );
		// Exit if it's nullptr.
		if ( !targetNameEntity ) {
			break;
		}
