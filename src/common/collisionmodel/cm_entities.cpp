...
static cm_entity_parsed_type_t CM_ParseKeyValueType( cm_entity_t *pair ) {
    ...
    if ( pair->nullable_string != nullptr && COM_IsInt( pair->nullable_string ) ) {
        // Succesfully parsed integer value, 
        if ( ( sscanf( pair->nullable_string, "%d", &pair->integer ) == 1 ) ) {
            pair->parsed_type = static_cast<cm_entity_parsed_type_t>( pair->parsed_type | cm_entity_parsed_type_t::ENTITY_PARSED_TYPE_INTEGER );
        } else {
    ...
    //        pair->parsed_type = static_cast<cm_entity_parsed_type_t>( pair->parsed_type | cm_entity_parsed_type_t::ENTITY_PARSED_TYPE_FLOAT );
    //    } else {
    //        Com_Error( ERR_DROP, "%s: EOF without closing brace", __func__ );
    //    }
    ...
    if ( pair->nullable_string != nullptr ) {
        ...
        if ( components >= 1 ) {
            for ( int32_t i = 1; i <= components; i++ ) {
                ...
                case 2:
                    ...
                    pair->parsed_type = static_cast<cm_entity_parsed_type_t>( pair->parsed_type | cm_entity_parsed_type_t::ENTITY_PARSED_TYPE_VECTOR2 );
                    break;
                case 3:
                    pair->vec2[ 0 ] = pair->vec3[ 0 ] = pair->vec4[ 0 ] = parsedVector[ 0 ];
                    pair->vec2[ 1 ] = pair->vec3[ 1 ] = pair->vec4[ 1 ] = parsedVector[ 1 ];
                    ...
                    pair->parsed_type = static_cast<cm_entity_parsed_type_t>( pair->parsed_type | cm_entity_parsed_type_t::ENTITY_PARSED_TYPE_VECTOR2
                        | cm_entity_parsed_type_t::ENTITY_PARSED_TYPE_VECTOR3 );
                    break;
                case 4:
                    pair->vec2[ 0 ] = pair->vec3[ 0 ] = pair->vec4[ 0 ] = parsedVector[ 0 ];
                    pair->vec2[ 1 ] = pair->vec3[ 1 ] = pair->vec4[ 1 ] = parsedVector[ 1 ];
                    ...
                    pair->parsed_type = static_cast<cm_entity_parsed_type_t>( pair->parsed_type | cm_entity_parsed_type_t::ENTITY_PARSED_TYPE_VECTOR2
                        | cm_entity_parsed_type_t::ENTITY_PARSED_TYPE_VECTOR3 | cm_entity_parsed_type_t::ENTITY_PARSED_TYPE_VECTOR4 );
                    break;
                }
            }
        ...
        pair->parsed_type = static_cast<cm_entity_parsed_type_t>( pair->parsed_type | cm_entity_parsed_type_t::ENTITY_PARSED_TYPE_NONE );
    }
...
static cm_entity_t *CM_ParseKeyValuePair( const char **entityString ) {
    ...
    while ( 1 ) {
        ...
        if ( Q_strnlen( pair->string, MAX_VALUE ) ) {
            pair->parsed_type = static_cast<cm_entity_parsed_type_t>( pair->parsed_type | ENTITY_PARSED_TYPE_STRING );
