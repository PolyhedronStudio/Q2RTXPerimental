...
int MOD_LoadIQM_Base( model_t *model, const void *rawdata, size_t length, const char *mod_name ) {
	...
	if ( header->version != IQM_VERSION ) {
		Com_WPrintf( "MOD_LoadIQM_Base: %s is a unsupported IQM version (%d), only version %d is supported.\n",
	...
	if ( header->num_joints > IQM_MAX_JOINTS ) {
		Com_WPrintf( "MOD_LoadIQM_Base: %s has more than %d joints (%d).\n",
	...
	if ( header->num_meshes ) {
		...
		if ( vertexArrayFormat[ IQM_POSITION ] == -1 || vertexArrayFormat[ IQM_NORMAL ] == -1 || vertexArrayFormat[ IQM_TEXCOORD ] == -1 ) {
			Com_WPrintf( "MOD_LoadIQM_Base: %s is missing IQM_POSITION, IQM_NORMAL, and/or IQM_TEXCOORD array.\n", mod_name );
			return Q_ERR_INVALID_FORMAT;
		}

		if ( header->num_joints ) {
			if ( vertexArrayFormat[ IQM_BLENDINDEXES ] == -1 || vertexArrayFormat[ IQM_BLENDWEIGHTS ] == -1 ) {
				Com_WPrintf( "MOD_LoadIQM_Base: %s is missing IQM_BLENDINDEXES and/or IQM_BLENDWEIGHTS array.\n", mod_name );
				return Q_ERR_INVALID_FORMAT;
			}
		} else {
	...
	if ( header->num_poses != header->num_joints && header->num_poses != 0 ) {
		Com_WPrintf( "MOD_LoadIQM_Base: %s has %d poses and %d joints, must have the same number or 0 poses\n",
